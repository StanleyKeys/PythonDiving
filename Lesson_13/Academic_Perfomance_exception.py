...
class MyTitleError(Exception):
    def __init__(self, value):
        self.txt = f'{value} must be with a Title chars'


class MyAlphaError(Exception):

    def __init__(self, value):
        self.txt = f'{value} must be with only alphabet symbols'
        ...
        for word in string_list:
            if word.isalpha():
                if word.istitle():
                    ...
                    raise MyTitleError(value)
            else: 
                raise MyAlphaError(value)
        self.__fio = value
